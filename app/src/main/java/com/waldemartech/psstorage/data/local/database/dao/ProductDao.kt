...
interface ProductDao {
    ...
    @Query("SELECT * FROM price_history INNER JOIN product ON productId = productIdPriceRef " +
    ...
    @Query("SELECT COUNT(*) FROM price_history INNER JOIN product ON productId = productIdPriceRef " +
            "WHERE storeIdInProduct == :storeId " +
            "AND productId NOT IN (SELECT favoriteProductId FROM favorite_product) " +
            "AND productId NOT IN (SELECT ignoredProductId FROM ignored_product) " +
            "AND dealIdPriceRef = :dealId")
        suspend fun loadProductCount(
            storeId: String,
            dealId: String
        ): Int
